    ...
    public class RxGenerator : ISourceGenerator
    {
                    ...
                    receiver))
            {
                ...
                switch (argumentTypes.Count)
                {
                    ...
                    default:
                        ...
                        string objStr = string.Join(", ", argumentTypes.Select((_, i) => $"obj{i}"));
            ...
            GetExtensionMethodInfo(GeneratorExecutionContext context, SyntaxReceiver receiver)
        {
            ...
            foreach (MemberAccessExpressionSyntax syntax in receiver.GenerateCandidates)
            {
                ...
                if (typeSymbol.GetMembersOfType<IEventSymbol>().FirstOrDefault(m => m.Name == eventName) is not { } ev)
                 continue;

                if (ev.Type is not INamedTypeSymbol namedTypeSymbol) continue;
